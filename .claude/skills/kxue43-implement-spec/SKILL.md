...
Read `$spec_path` with `Read` and implement it in full on the current branch, using `Read`/`Grep`/`Bash` (including `git log`/`git blame` for history and authorship context) to explore the codebase as needed and `Write`/`Edit` to make changes. If `$focus_prompt` is present, keep it in mind throughout the implementation as described above. Commit work at logical checkpoints with clear, descriptive commit messages that explain why. Use your own judgment on how many commits to make and when to make them — do not ask the user for permission before each individual commit.
