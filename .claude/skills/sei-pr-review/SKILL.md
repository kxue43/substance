...
## Shared Procedure: Verify SHA

Used by both `start` (Review Process step 1) and `followup` (Step 1) to confirm the local HEAD
matches the PR's remote branch before any review work begins. Takes no arguments — `verify-sha`
derives the branch to check from the current git state.
...
URL, invoke the `sei-fetch-single-comment` skill with that URL to obtain its content.

### Review Process

1. **Verify the local HEAD matches the PR's remote branch.**
   Follow the Shared Procedure: Verify SHA above.
...
| Variable | Description |
|----------|-------------|
| `$session_file` | Path relative to CWD of the session manifest file generated by a previous `/sei-pr-review start` run |
...
`sei-fetch-single-comment` skill with that URL to obtain its content.

### Step 1 — Verify SHA

Before doing anything else, follow the Shared Procedure: Verify SHA above. If it fails,
...
4. The **last entry** in the `load:` list is the **report file** — the single report file used
   ...
   Note its `pr_url` front-matter field — needed in Step 3 below to fetch reviewer comments.
...
you will issue a warning in the Step 4 brief report (do not record it in the report file).
...
Use this diff as the primary source of changes for both review tracks below.

#### Perform the review tracks

Perform the following two review tracks **in order, completing the first before starting the second**:

1. **Comment resolution:** The reviewer's posted comments carry higher weight than the finding
   summary written by Claude Code — when assessing whether a concern is addressed, prioritize
   what the reviewer actually asked for in their comment over the original finding description.
   For each label in `changes_requested` with a matching reviewer comment (per pre-review
   preparation), rate how well the new code changes address it:
   - **Addressed:** the reviewer's concern is fully resolved.
   ...
   Partially addressed counts as **unaddressed** in the session summary, but its finding summary
   should be **rewritten** to reflect the partial progress; fully unaddressed findings keep their
   summary unchanged. Labels with no matching comment were already classified fully unaddressed in
   pre-review preparation — skip rating them here. Do not revisit labels in `findings_dismissed` —
...
**Make a brief report to the user** covering the two review tracks:
...
**If everything is fully addressed and no new findings were surfaced:**
...
**If some findings are not fully addressed, or new findings were surfaced:**
...
- **Unaddressed findings:** Keep their label in `changes_requested` and keep their finding
  summary unchanged.
- **New findings:** Use `next_labels` from the session manifest's front
